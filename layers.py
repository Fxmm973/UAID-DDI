...
class PharmacophoreAwareTransformerConv(nn.Module):
    ...
    def detect_pharmacophore(self, x):
        ...
        score[:, 0] = x[:, 1] + x[:, 2]  # N,O原子作为氢键供体特征
        score[:, 1] = x[:, 2] + x[:, 1]  # O,N原子作为氢键受体特征
        score[:, 2] = x[:, 0]  # C原子作为疏水特征
        score[:, 3] = x[:, 54] if x.size(1) > 54 else 0.#芳香性特征（需特征维度>54
        score[:, 4] = torch.abs(x[:, 45]) if x.size(1) > 45 else 0.   #电荷绝对值作为带电特征
        return score
