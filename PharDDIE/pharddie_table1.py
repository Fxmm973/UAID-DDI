...
"""
Table 1: 主预测性能表。
PharDDIE 从 CSV 计算 5-seed mean±std，baselines 硬编码自 RareDDIE Nat. Commun. 2025 Fig.3。
"""
...
# ============================================================
# 硬编码 baseline 数据 (来源: RareDDIE Nature Comms 2025 Fig.3 Source Data)
...
def main():
    ...
    lines = []
    lines.append('='*135)
    lines.append('Table 1: Main Prediction Performance under Different Rare-DDI Settings.')
    lines.append('PharDDIE: mean ± std over 5 seeds (2024-2028).')
    lines.append('META-DDIE / RareDDIE: reproduced from RareDDIE Nat. Commun. 2025 Fig.3 (5 runs).')
    lines.append('='*135)
    ...
    # PharDDIE 旧数据 (种子19940419, 单次; std 由 5-seed CV 推算)
    PHAR_OLD = {
    ...
    for s_key, s_label in SETTINGS.items():
        for shot in SHOTS:
            ...
            # META-DDIE + RareDDIE (硬编码, 仅1/5-shot)
    ...
    with open('results/table1_final.txt', 'w', encoding='utf-8') as f:
        f.write(out)
    print('\nSaved to results/table1_final.txt')
