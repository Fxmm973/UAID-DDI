...
class Trainer(object):
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,
            ...
            query_scores, loss2_p = self.matcher(query, support, query_meta, support_meta, query_batch, support_batch,
                                                  self.optim_VAE)
            false_scores, loss2_n = self.matcher(false, support, false_meta, support_meta, false_batch, support_batch,
                                                  self.optim_VAE)
            ...
            loss += 0.5 * (loss2_p + loss2_n)  # symmetric SRAE loss over pos & neg queries
        ...
        for query_ in test_tasks.keys():
            ...
            for i in range(len(query_triples_rel2id) // test_size + 1):
                ...
                if meta:
                    ...
                    scores.detach()
                    probas_pred_t.append(torch.sigmoid(scores.detach()).cpu().numpy())
                else:
    ...
    def test_(self):
        ...
        logging.info('=' * 50)
        common_test_auc = self.eval_acc(meta=self.meta, mode='common_test')

        logging.info('\n' + '=' * 50)
        logging.info('TESTING ON TEST SET')
        logging.info('=' * 50)
        test_auc = self.eval_acc(meta=self.meta, mode='test')

        logging.info('\n' + '=' * 50)
        logging.info('TESTING ON TEST2 SET')
        logging.info('=' * 50)
        ...
        logging.info('=' * 60)
        logging.info(f'Common Test Set (sample size > 50): AUROC = {common_test_auc:.4f}')
        logging.info(f'Test Set: AUROC = {test_auc:.4f}')
        ...
        return {
            'common_test_auc': common_test_auc,
            'test_auc': test_auc,
            'test2_auc': test2_auc
        }
