...
def train_generate_(dataset, batch_size, few, symbol2id, ent2id, e1rel_e2, num_neg=1):
    ...
    while True:
        ...
        for triple in query_triples:
            ...
            candidates_filtered = [c for c in candidates if c != e_t]

            if len(candidates_filtered) >= num_neg:
                noises = random.sample(candidates_filtered, num_neg)
            else:
                noises = candidates_filtered
