...
class Trainer(object):
    ...
    def get_meta(self, left, right):
        left_connections = Variable(
            torch.LongTensor(np.stack([self.connections[_, :, :] for _ in left], axis=0))).to(DEVICE)
        left_degrees = Variable(torch.FloatTensor([self.e1_degrees[_] for _ in left])).to(DEVICE)
        right_connections = Variable(
            torch.LongTensor(np.stack([self.connections[_, :, :] for _ in right], axis=0))).to(DEVICE)
        right_degrees = Variable(torch.FloatTensor([self.e1_degrees[_] for _ in right])).to(DEVICE)
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,
            ...
            query_scores, loss2 = self.matcher(task_emb, query, support, query_meta, support_meta, query_batch,
                                               support_batch, self.optim_VAE, trainGAN=False)
            lT[1] = loss2.detach()
            false_scores, loss2 = self.matcher(task_emb, query, support, false_meta, support_meta, false_batch,
                                               support_batch, self.optim_VAE, trainGAN=False)
            lT[2] = loss2.detach()
            probas_pred.append(np.concatenate(
                [torch.sigmoid(query_scores.detach()).cpu(), torch.sigmoid(false_scores.detach()).cpu()]))
            ground_truth.append(
                np.concatenate([np.ones(query_scores.shape[0]), np.zeros(false_scores.shape[:2]).reshape(-1)]))
    ...
    def eval_acc(self, mode='dev', meta=False):
        ...
        if mode == 'dev':
            ...
            test_tasks = json.load(open(self.dataset + '/test1_tasks.json'))
        else:
        ...
        for query_ in test_tasks.keys():
            ...
            query = Variable(torch.LongTensor(query_pairs)).to(DEVICE)
