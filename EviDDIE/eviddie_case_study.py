...
logging.info(f'Loaded {len(db_pairs)} pairs from {dti_path}')
...
print(f'{"=" * 100}')
print(f'{"Rank":<6} {"Drug1":<10} {"Drug2":<10} {"Event":<45} {"Prob":<10} {"Uncertainty":<12} {"Score r":<10} {"DB match":<12} {"Action":<15}')
print('-' * 100)

K = min(8, len(results))
for rank in range(K):
    p, u, r, evt, d1, d2, in_db = results[rank]
    source = DRUGBANK_VERSION if in_db else 'Not identified'
    if p >= TAU_P and u < TAU_U:
        ...
        action = 'Low priority'
    print(f'{rank + 1:<6} {d1:<10} {d2:<10} {evt[:45]:<45} {p:<10.4f} {u:<12.4f} {r:<10.4f} {"YES" if in_db else "no":<12} {action:<15}')
