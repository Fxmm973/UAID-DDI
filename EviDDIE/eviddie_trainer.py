    ...
    def eval_acc(self, mode='dev', meta=False):
        ...
        for query_ in tqdm(test_tasks.keys(), desc="Evaluating Tasks"):
            ...
            for i in range(0, len(all_pairs_ids), test_size):
                batch_pairs = torch.LongTensor(all_pairs_ids[i: i + test_size]).cuda()
...
class EvidentialLoss(nn.Module):
    ...
    def forward(self, alpha, target_type, batch_num):
        ...
        if target_type == 'pos':
            y = torch.tensor([[0, 1]], dtype=torch.float32).repeat(alpha.size(0), 1).cuda()
        else:
            y = torch.tensor([[1, 0]], dtype=torch.float32).repeat(alpha.size(0), 1).cuda()
