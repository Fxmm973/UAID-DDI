#!/usr/bin/env python
"""Case Study: Dataset 2 rare-event 推理 + 跨split审计 + DrugBank 查找，输出 Table 5"""
...
# ---- 审计：加载所有 split 的 unordered pairs ----
all_pairs = set()
for split_name in ['train_tasks', 'dev_tasks', 'test_tasks']:  # exclude train/dev/test, allow test2
...
# ---- Output top-K ----
print(f'\n{"="*100}')
print(f'Table 5: Post Hoc Database Concordance Examples (top candidates)')
print(f'Total candidates after cross-split filtering: {len(results)}')
print(f'{"="*100}')
...
print(r'\centering')
print(r'\caption{Post hoc database concordance examples: top-ranked predictions from the rare-event split of Dataset~2 after cross-split filtering.}')
